...
class IndexTestCase(TransactionTestCase):
    ...
    def test_userobjectpermission_query_uses_index(self):
        ...
        with self.assertNumQueries(1):
            permissions = list(UserObjectPermission.objects.filter(
                user=self.user,
                content_type=self.content_type,
                object_pk=str(self.project.pk)
            ))
    ...
    def test_groupobjectpermission_query_uses_index(self):
        ...
        with self.assertNumQueries(1):
            permissions = list(GroupObjectPermission.objects.filter(
                group=self.group,
                content_type=self.content_type,
                object_pk=str(self.project.pk)
            ))
...
class GuardianShortcutsPerformanceTestCase(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.users = [
            User.objects.create_user(username=f"perfuser{i}", email=f"perfuser{i}@example.com")
            for i in range(50)
        ]
        
        cls.groups = [
            Group.objects.create(name=f"perfgroup{i}")
            for i in range(20)
        ]
        
        cls.projects = [
            Project.objects.create(name=f"Performance Project {i}")
            for i in range(100)
        ]
        ...
        for user in cls.users:
            ...
            for project in user_projects:
                ...
                perms = random.sample([cls.add_permission, cls.change_permission, cls.delete_permission], 
                                    random.randint(1, 3))
                for perm in perms:
                    UserObjectPermission.objects.create(
                        user=user,
    ...
    def test_get_objects_for_user_multiple_permissions(self):
        ...
        projects_with_any = get_objects_for_user(
            user, 
            ['testapp.add_project', 'testapp.change_project'], 
            Project,
            any_perm=True
        )
    ...
    def test_get_objects_for_user_with_groups_performance(self):
        ...
        projects_with_change = get_objects_for_user(
            user, 
    ...
    def test_bulk_permission_check_performance(self):
        ...
        allowed_projects = get_objects_for_user(
            user, 
            'testapp.change_project', 
            Project.objects.filter(pk__in=[p.pk for p in projects_to_check])
        )
...
class IndexVsNoIndexPerformanceTestCase(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.users = [
            User.objects.create_user(username=f"timinguser{i}", email=f"timinguser{i}@example.com")
            for i in range(100)
        ]
        
        cls.groups = [
            Group.objects.create(name=f"timinggroup{i}")
            for i in range(50)
        ]
        
        cls.projects = [
            Project.objects.create(name=f"Timing Project {i}")
            for i in range(500)
        ]
    ...
    def test_permission_existence_at_scale_timing(self):
        ...
        self.assertLess(elapsed_time, 0.5, f"Permission existence checks should be fast with indexes")
